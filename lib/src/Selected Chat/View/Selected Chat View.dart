  ...
  buildMessageList() {
    return StreamBuilder(
        stream: chatService.getMessages(
            widget.selectedUser['uid'], firebaseAuthinst.currentUser!.uid),
        builder: (context, snapshot) {
          ...
          return ListView.builder(
              ...
              itemBuilder: (cnt, index) {
                var allign = (snapshot.data!.docs[index]["senderId"] ==
                        firebaseAuthinst.currentUser?.uid)
                    ? MainAxisAlignment.end
                    : MainAxisAlignment.start;
                var bgclr = (snapshot.data!.docs[index]["senderId"] ==
                        firebaseAuthinst.currentUser?.uid)
                    ? Color(0xff007665)
                    : Colors.grey;
                return Row(
                  mainAxisSize: MainAxisSize.min,
                  mainAxisAlignment: allign,
                  children: [
                    Padding(
